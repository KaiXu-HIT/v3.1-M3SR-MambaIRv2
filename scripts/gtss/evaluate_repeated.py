"""Three-seed GTSS/RGB evaluation; optionally add frozen v3.0 GRS.
...
def summarize(runs, seeds):
    ...
    lines = ['# GTSS-MambaSR paired evaluation', '',
    ...
    def delta(dataset):
        return report['summary'][dataset]['gtss']['delta_psnr_vs_rgb']
    avg = report['average_delta_vs_rgb']['gtss']
    report['gtss_criteria'] = dict(average_positive=avg > 0, average_target_met=avg >= 0.05,
        urban100_target_met=delta('Urban100') >= 0.08,
        manga109_target_met=delta('Manga109') >= 0.10, b100_delta_psnr=delta('B100'))
    lines.extend(['', f'GTSS five-set average delta vs RGB: {avg:+.4f} dB.',
                  'Criteria: '+json.dumps(report['gtss_criteria'], sort_keys=True)])
    return report, '\n'.join(lines)+'\n'


def main():
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument('--baseline-config', default='options/test/mambairv2/test_GRS_RGB_reference_x4.yml')
    ...
    p.add_argument('--gtss-checkpoint')
    p.add_argument('--grs-checkpoint', help='Fill after GTSS training, before the three-model comparison.')
    ...
    p.add_argument('--output', default='results/gtss_comparison')
    ...
    for label in labels:
        ...
        if not ckpt or str(ckpt).startswith('__SET_'):
            p.error(f'{label}: checkpoint is reserved. AFTER GTSS training, fill '
                    f'{cfgpath}: path.pretrain_network_g or pass --{label}-checkpoint PATH '
                    'before the three-model comparison. Training and GTSS-only testing need no GRS weights.')
    ...
    for seed in a.seeds:
        for label, cfg in configs.items():
            ...
            config['name'] = f'GTSS_fair_{label}_seed{seed}'
