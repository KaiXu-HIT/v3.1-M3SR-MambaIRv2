...
class GTSSMambaIRv2Model(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        # GTSS first experiment: preserve baseline optimizer/scheduler; L1 only.
        if self.cri_pix is None or self.cri_perceptual is not None:
            raise ValueError('GTSS first experiment requires pixel loss only.')
    ...
    def test_selfensemble(self):
        raise NotImplementedError('Use baseline-matched partition inference for GTSS evaluation.')
    ...
    def test(self):
        ...
        # GTSS: apply exactly the RGB partition padding to depth.
